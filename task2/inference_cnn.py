...
def argument_parser():
    ...
    parser.add_argument("--train_data", required=True, type=str, help="Provide a path to training data") # get the pass to initialize classes names for prediction
    parser.add_argument("--model_dir", required=True, type=str, help="Provide a path to a model") # path to the trained model
...
def initialize_model(path):
    ...
    model = torch.load(f"{path}", weights_only=False) # load model
...
def main():
    ...
    prediction = predict(args.img_path, args.train_data, model, transform)
    return prediction
